...
- **使用不可プリント（`not_tournament_legal`）は「最安値」の対象外**：金縁/銀縁（World
  Championship Decks、Un-set等）・memorabilia区分（30th Anniversary Edition、Collectors'
  Edition等）は、オラクルとしてはトーナメント合法でもこの物理プリント自体は使用不可のため、
  オラクル単位の「全プリント中最安値」計算には含めない。含めると、実際には買えない/使えない
  超高額な特殊版が「最安値」として表示されてしまう（Black Lotus + Collectors' Editionで
  実際に発生、`docs/incident-log.md`参照）。この除外は日次の実データ計算・過去の一括
  バックフィルの両方で一貫して適用する必要がある（過去に一括バックフィル側だけ実装漏れが
  あった）
- **除外した結果、他に候補が無い日は「価格データ無し」として扱う**：低流動性カード
  （Reserved List級の稀少カード等）は、その日の価格データが使用不可プリントにしか
  無いことがある。その場合は代替値を探さず、素直に欠測（null）として扱う（不正確な値を
  無理に埋めない）
...
| レイアウト | キー例 | 用途 | 更新頻度 |
|---|---|---|---|
| カード単位（オラクル） | `oracle-history/{oracle_id}.ndjson.gz` | サイトの価格推移グラフ・ランキング詳細計算 | 日次、前日比で値が変わった分のみ書き込み |
| カード単位（プリント） | `print-history/{scryfall_id}.ndjson.gz` | プリント別の価格表示 | 日次、差分のみ |
| 事前計算キャッシュ | `price-changes/latest.ndjson.gz` | ランキング/トレンドページ（全オラクル分を1ファイルで、直近7日分の系列付き） | 日次、全件洗い替え |
| 月次バルク（オラクル・プリント） | `price-history/YYYY-MM.ndjson.gz`・`print-price-history/YYYY-MM.ndjson.gz` | バッチ集計専用（streak計算等、CPU時間制限の無いGitHub Actions側でのみ読む） | 日次追記 |

サイト（Cloudflare Workers）が毎リクエストで読むファイルは「カード単位」または
「事前計算キャッシュ」のみに限定し、月次バルク（全カード横断、各5〜8MB圧縮）は絶対に
読まない。月次バルクを毎リクエスト読む設計にすると、Workers無料枠のCPU時間制限
（10ms/リクエスト）を超過する（`docs/incident-log.md`のhyparquet事故と同種のリスク）。

**書き込みは差分方式が基本**：R2の書き込み（Class A操作）は無料枠が月100万リクエストと
限られるため、値が変わったカードだけ書き込む。書き込み関数（`mergeOraclePriceRows`/
`mergePrintPriceRows`）自体にも「既存データと同じなら実際には書き込まない」チェックが
入っており、呼び出し元が差分判定をサボっても二重に守られる。ただし**呼び出し元は必ず
全データをまとめてから1回だけ呼ぶこと**（月ごと・ページごとにループの中で呼ぶと、
書き込み関数の内部チェックだけでは救えない非効率が発生する。実際に2回同じミスをして数時間
無駄にしたため、3回以上同一プロセス内で呼ぶと例外を投げるガードを入れてある）

**カタログ（デッキ未使用カード）の価格更新は3日に1回**：追跡カード（デッキで使われたことが
あるカード）は毎日更新するが、カタログ全体（4万件超のプリント）を毎日差分チェックすると
R2書き込み無料枠を超過する。デッキ未使用カードは閲覧頻度が低く数日の鮮度遅れが実害に
ならないため、間引いている（`scripts/snapshot-catalog-prices.mjs`）

### 継続注目カード（`card_streaks`）の計算方式

前日の計算結果を引き継がない**完全ステートレス方式**。日次バッチ側で、直近
`STREAK_LOOKBACK_DAYS`（30日）分を月次バルクファイルから読み、当日から暦日ベースで
（配列のインデックスではなく実際の日付が1日前かどうかで）遡って、その場で毎回ゼロから
計算し直す。前日の値を信頼する「引き継ぎ方式」は、1日でも計算が飛ぶと以降ズレを
引きずるリスクがあるため採用しなかった（`docs/incident-log.md`参照）。

### データソースの補足

- **取引量（販売数量）データは無料では取得不可**：TCGPlayer自体が無料の公開APIで
  提供していない。有料の第三者API（TCGAPIs.com等）が実質唯一の選択肢で、launch時点では
  対象外（2章参照）
- **デッキ未使用カードのカタログ価格取得はScryfall bulk dataに統一**：以前はTCGCSVを
  別データソースとして使っていたが、追跡カード用に既にダウンロード・キャッシュしている
  Scryfall bulk dataが実質「全カード」の価格を持っているため、別ソースを使う理由が無かった
  （TCGCSVは2024-02〜2026-07-24の一括バックフィルにのみ使用、日次では使わない）
