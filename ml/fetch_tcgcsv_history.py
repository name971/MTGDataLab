...
def sync_month_to_r2(s3, key: str, new_rows: pd.DataFrame, id_column: str) -> None:
    """R2上の月次Parquetファイルを読み込み（無ければ空扱い）、新しい行とマージして書き戻す。
...
def main() -> None:
    ...
    with ProcessPoolExecutor(
        ...
        for month, days in sorted(days_by_month.items()):
            ...
            if month_buffer:
                sync_month_to_r2(s3, f"{R2_PREFIX}/{month}.parquet", pd.DataFrame(month_buffer), "oracle_id")
            if print_month_buffer:
                sync_month_to_r2(s3, f"{R2_PRINT_PREFIX}/{month}.parquet", pd.DataFrame(print_month_buffer), "scryfall_id")
